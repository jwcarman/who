...
/**
 * JDBC implementation of {@link RolePermissionRepository} backed by the {@code who_role_permission}
 * table.
 */
@Repository
public class JdbcRolePermissionRepository implements RolePermissionRepository {
  ...
  public Set<String> findPermissionsByRoleIds(Collection<UUID> roleIds) {
    ...
    return new HashSet<>(
        jdbcClient
            .sql(
                "SELECT DISTINCT permission FROM who_role_permission WHERE role_id = ANY(:roleIds)")
  ...
  public void addPermission(UUID roleId, String permission) {
    jdbcClient
        .sql(
            "INSERT INTO who_role_permission (role_id, permission) VALUES (:roleId, :permission) ON CONFLICT DO NOTHING")
        .param(PARAM_ROLE_ID, roleId)
