...
/**
 * JDBC implementation of {@link ApiKeyCredentialRepository} backed by the {@code
 * who_api_key_credential} table.
 */
@Repository
public class JdbcApiKeyCredentialRepository implements ApiKeyCredentialRepository {
  ...
  private static final RowMapper<ApiKeyCredential> ROW_MAPPER =
      (rs, rowNum) ->
          new ApiKeyCredential(
              rs.getObject(COL_ID, UUID.class), rs.getString(COL_NAME), rs.getString(COL_KEY_HASH));
  ...
  public void deleteById(UUID id) {
    jdbcClient.sql("DELETE FROM who_api_key_credential WHERE id = :id").param(COL_ID, id).update();
  }
}
