...
/**
 * Initializes Who module database schemas by running each module's bundled DDL script
 * against the configured {@link DataSource}.
 *
 * <p>Only modules present on the classpath contribute scripts. All scripts use
 * {@code CREATE TABLE IF NOT EXISTS}, so they are safe to run on every startup.
 *
 * <p>Controlled by the {@code who.initialize-schema} property
 * ({@code always}, {@code embedded}, {@code never}; default {@code embedded}).
 */
public class WhoDataSourceScriptDatabaseInitializer extends DataSourceScriptDatabaseInitializer {

    private static final List<ModuleSchema> MODULE_SCHEMAS = List.of(
            new ModuleSchema("org.jwcarman.who.jdbc.JdbcIdentityRepository",
                    "classpath:org/jwcarman/who/jdbc/schema.sql"),
            new ModuleSchema("org.jwcarman.who.rbac.JdbcRoleRepository",
                    "classpath:org/jwcarman/who/rbac/schema.sql"),
            new ModuleSchema("org.jwcarman.who.jwt.JdbcJwtCredentialRepository",
                    "classpath:org/jwcarman/who/jwt/schema.sql"),
            new ModuleSchema("org.jwcarman.who.enrollment.JdbcEnrollmentTokenRepository",
                    "classpath:org/jwcarman/who/enrollment/schema.sql"),
            new ModuleSchema("org.jwcarman.who.apikey.JdbcApiKeyCredentialRepository",
     ...
     */
    public WhoDataSourceScriptDatabaseInitializer(DataSource dataSource, DatabaseInitializationMode mode) {
